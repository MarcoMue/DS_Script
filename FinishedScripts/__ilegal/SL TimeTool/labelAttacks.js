...
(async () => {
  ...
  const serverOffset =
    Math.round(Math.abs(new Date(serverDateTime) - new Date()) / 36e5) *
    3600 *
    1000;
  ...
  Incomings = {
    ...
    updateSettings: function () {
      ...
      localStorage.setItem(
        game_data.world + "confirmenhancersettings",
    ...
    loadSettings: function () {
      var settings =
        JSON.parse(
          localStorage.getItem(game_data.world + "confirmenhancersettings")
        ) || {};
      if (
        localStorage.getItem(game_data.world + "confirmenhancersettings") ===
        null
    ...
    createTable: function (_callback) {
      ...
      var villageUrl = document
        .getElementById("command-data-form")
        .getElementsByClassName("village_anchor")[0]
      ...
      $.when(
        loadRunningCommands(villageUrl).done(function (html) {
          const commandsTable = $(html).find(".commands-container");
          var delay = document.createElement("delay");
          delay.innerHTML =
            "<div style='width:100%; height:20px'></div><div width='100%' style='display: none' id='delayBar'>delay: <input type='number' id='delay' style='width: 100px;'/>     remember: <input type='checkbox' id='remember2'/></div>";
          ...
          if (commandsTable.length > 0) {
            ...
            commandsTable.find("tr.command-row").each(function () {
              ...
              const sendTime =
                $(this).find("td:last span").data("endtime") * 1000 - duration;
              $(this).append(
                `<td class="sendTime"><b><span class="timer" style="color: darkblue" data-endtime="${
                  sendTime / 1000
                }"></span></b></b><br></td>`
              );
            });
          ...
          if (
            ...
            document.referrer.indexOf("arrivalTimestamp") > -1
          ) {
            let delay = $("#delay").val();
            if (!delay.trim().length) delay = 0;
            const arrivalTime = new Date(
              parseInt(previousUrlSearchParams.get("arrivalTimestamp")) +
                parseInt(delay)
            );
          ...
          handleTimers();
          if (typeof enableCommandRowClick === "function")
            window.enableCommandRowClick();

          if (timeBarWidth) {
            $(".village_anchor").removeClass("village_anchor");
            $("#command-data-form .vis:first, #date_arrival").width(
              timeBarWidth
            );
          }
        })
    ...
    fillSnipeTool: function (timeString, isTimestamp) {
      let date = timeString;
      if (!isTimestamp) {
        const [hour, min, sec, ms] = timeString
          .match(/\d+:\d+:\d+.\d+|\d+:\d+:\d+/)[0]
          .split(":");
        let [day, month, year] = $("#serverDate")
          .text()
          .split("/")
          .map((u) => Number(u));
        ...
        if (
          timeString.match(
            window.lang["57d28d1b211fddbb7a499ead5bf23079"].split(" ")[0]
          )
        ) {
          date = new Date(year, month - 1, day + 1, hour, min, sec, ms ?? 0);
        } else if (timeString.match(/\d+\.\d+\.\d+|\d+\.\d+|\d+\/\d+\/\d+/)) {
          [day, month, year] = timeString.includes("om")
            ? timeString.split("om")[0].match(/\d+/g)
            : timeString.match(/\d+/g);
          if (!year) year = new Date().getFullYear().toString();

          const yearPrefix = new Date(date)
            .getFullYear()
            .toString()
            .slice(0, 2);
      ...
      document.getElementById("timegoal").value = msToDatetimeLocal(
        date.getTime()
      );
  ...
  SnipeTool = {
    ...
    init: function () {
      ...
      progressBar.innerHTML =
        "<div id='progress_bar'><div id='time'></div><div id='bar'></div></div>";
      timeGoalInput.innerHTML =
        "<div width='100%'>snipe time: <input type='datetime-local' max=\"9999-12-31T23:59:59\" id='timegoal' step='1'></div>";
      msGoalInput.innerHTML =
        "<div width='100%'>snipe ms: <input type='number' id='msgoal' style='width: 100px;'/></div>";
      rememberInput.innerHTML =
        "<div width='100%'><label>remember: <input type='checkbox' id='remember'/></label><div id='watermark'>made by Ricardo/Bottenkraker.</div></div>";
      ...
      if (this.timeGoal !== 0) {
        ...
        stuur.innerHTML =
          "<td>Stuurtijd:</td><td>" +
          sendDate.toLocaleDateString(undefined, {
            day: "numeric",
            month: "numeric",
          }) +
          "&nbsp;" +
          ...
          "</b>" +
          "&nbsp;&nbsp;&nbsp;(<span class='timer' id='timer2' data-endtime='" +
          sendTime / 1000 +
          "'></span>)</td>";
    ...
    loadSettings: function () {
      var settings =
        JSON.parse(localStorage.getItem(game_data.world + "snipesettings")) ||
        {};
    ...
    updateBar: function () {
      ...
      var timestamp =
        element.innerHTML.match(/\w+\s+\w+\s+\d+:\d+:\d+/) ??
        element.innerHTML.match(/\w+\s+\d+\.\d+\.\s+\w+\s+\d+:\d+:\d+/);
  ...
  SnipeTool.addGlobalStyle(
    "#progress_bar {width: 100%; height: 20px; background-color: grey;}"
  );
  SnipeTool.addGlobalStyle(
    "#time {width: 100%; height: 20px; text-align: center; vertical-align: middle; padding-top:3px}"
  );
  SnipeTool.addGlobalStyle(
    "#bar {width: 0%; background: green; height: 20px; margin-top: -23px;}"
  );
  SnipeTool.addGlobalStyle(
    "#watermark {font-size: 6px; color: grey; text-align: right; margin-top: -10px;}"
  );
  ...
  if (document.getElementById("date_arrival")) {
    ...
    var x = new MutationObserver(function (e) {
      if (
        e[0].removedNodes &&
        document.getElementById("date_arrival") &&
        !script_started
      ) {
        script_started = true;
        startScript();
