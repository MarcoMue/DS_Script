...
if (document.URL.indexOf("screen=am_farm") == -1)
  ...
  $("img").each(function () {
    ...
    if (tempStr.indexOf("attack") != -1) $(this).addClass("tooltip");
  });
  ...
  if (changeVillageInterval !== false) {
    if (changeVillageInterval === true)
      ...
      else changeVillageInterval = randomTime(3000, 5000);
    else
      changeVillageInterval =
        parseInt(changeVillageInterval) + parseInt(randomTime(500, 1000));
  }
  ...
  if (game_data.screen == "am_farm") {
    ...
    $(".paged-nav-item:not(:first-child)").each(function () {
      $.ajax({
        ...
        error: function () {
          UI.ErrorMessage(
            "An error occurred while downloading data. Refresh the page to try again",
            5000
          );
    ...
    UI.InfoMessage(
      "The script should be used from the farm assistant view.",
      2000,
      "error"
    );
  }
  lootAssistantPageSize = document.querySelectorAll(
