...
function getPlayerDict() {
  ...
  localStorage.playerDictFake =
    server + ":::" + now + ":::" + JSON.stringify(playerDict);
  return playerDict;
}
...
function addFilter() {
  ...
  if (filters[document.getElementById("variable").value]) {
    if (isNaN(document.getElementById("value").value)) {
      ...
      filters[document.getElementById("variable").value].push([
    ...
    if (isNaN(document.getElementById("value").value)) {
      ...
      filters[document.getElementById("variable").value] = [
        [
          document.getElementById("kind").value,
          document.getElementById("value").value,
        ],
...
function showData(data) {
  html =
    "<head></head><body><p><h2>Tribe data</h2>Mode selected: " +
    mode +
    "</p><p><textarea readonly=true>" +
    data +
    '</textarea></p><p><input type="button" class="btn evt-confirm-btn btn-confirm-yes" id="download" onclick="download(\'tribe info\',data)" value="Download as csv"></input><input type="button" class="btn evt-confirm-btn btn-confirm-no" onclick="openUI()" value="Back to main menu"></input></p></body>';
