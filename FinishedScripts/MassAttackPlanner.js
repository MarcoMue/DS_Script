...
const coordsToID = JSON.parse(localStorage.getItem(`coordsToID_${activeWorldData['world']}`));
let groups = [];
...
function groupScreen() {
    ...
    arrivalDateLabel.textContent = `Arrival Date for group #${groups.length + 1}:`;
...
function addAction() {
    ...
    if (actionName.length == 0 || fromCoords == null || toCoords == null || fromCoords.length < toCoords.length) {
        alert('Please enter valid data\nAction name can\'t be empty and from coords >= to coords > 0');
    }
    else {
        groups[groups.length - 1][1].push([actionName, slowestUnit, fromCoords, toCoords]);
...
function distanceInSec(fromCoord, toCoord, speed) {
    ...
    return Math.round(Math.sqrt((parseInt(fromCoordXY[0]) - parseInt(toCoordXY[0])) ** 2 + (parseInt(fromCoordXY[1]) - parseInt(toCoordXY[1])) ** 2) * speed);
}
...
function entryTemplate(sendDate, toCoord, arrivalDate, unit, actionName, link) {
    let strSendDate = sendDate.toISOString().match(/([^T]+)/)[0] + ' ' + sendDate.toTimeString().match(/([^ ]+)/)[0];
    let strArrivalDate = arrivalDate.toISOString().match(/([^T]+)/)[0] + ' ' + arrivalDate.toTimeString().match(/([^ ]+)/)[0];
    ...
    groups.forEach(group => {
        group[1].forEach(action => {
            ...
            action[3].forEach(toCoord => {
                ...
                // @ts-ignore
                let sendDate = new Date(group[0].getTime() - shortestDistance[0] * 1000);
                let link = getLink(shortestDistance[1], toCoord);
                let planEntry = entryTemplate(sendDate, toCoord, group[0], action[1], action[0], link);
