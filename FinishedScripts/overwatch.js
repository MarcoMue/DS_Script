javascript: var development = false;
//declaring globals
var options;
...
var currentCoords = "";
this.watchtowerRadius = [
  1.1, 1.3, 1.5, 1.7, 2, 2.3, 2.6, 3, 3.4, 3.9, 4.4, 5.1, 5.8, 6.7, 7.6, 8.7,
  10, 11.5, 13.1, 15,
];
...
$.getScript(
  "https://cdnjs.cloudflare.com/ajax/libs/jscolor/2.4.5/jscolor.min.js"
);
...
$.get("/game.php?screen=ally&mode=members_defense", function (membersDef) {
  ...
  playerIDs.forEach((element) => {
    urls.push(
      "/game.php?screen=ally&mode=members_defense&player_id=" + element
    );
  });
}).done(function () {
  //collect building sharing
  $.get(
    "/game.php?screen=ally&mode=members_buildings",
    function (membersBuildings) {
      buildingOptions = $(membersBuildings).find(
        ".input-nicer option:not(:first)"
      );
      ...
      playerBuildingIDs.forEach((element) => {
        buildingUrls.push(
          "/game.php?screen=ally&mode=members_buildings&player_id=" + element
        );
      });
    }
  ).done(function () {
    ...
    (i, data) => {
      ...
      for (let i = 0; i < table.length / 2; i++) {
        ...
        game_data.units.forEach((element, j) => {
          unitsInVillage[element] =
            table[i * 2].children[j + 2].innerText.trim();
          unitsEnroute[element] =
            table[i * 2 + 1].children[j + 1].innerText.trim() == "?"
              ? 0
              : table[i * 2 + 1].children[j + 1].innerText.trim();
          ...
          currentPop =
            currentPop +
            unitsInVillage[element] * unitPopValues[game_data.units[j]];
          totalPop =
            totalPop +
            unitsInVillage[element] * unitPopValues[game_data.units[j]] +
            parseInt(unitsEnroute[element]) * unitPopValues[game_data.units[j]];
        });
        if (hasIncomings) {
          attacksToVillage =
            table[i * 2].children[2 + game_data.units.length].innerText.trim();
        } else {
          attacksToVillage = "---";
        }
      ...
      $.getAll(
        ...
        (j, buildingTable) => {
          if ($(buildingTable).find(".table-responsive")) {
            if (
              $(buildingTable).find(
                '#ally_content img[src*="buildings/watchtower.png"]'
              ).length > 0
            ) {
              let cellIndex = $(buildingTable)
                .find('#ally_content img[src*="buildings/watchtower.png"]')
                .parent()
                .index();
              let wallIndex = $(buildingTable)
                .find('#ally_content img[src*="buildings/wall.png"]')
                .parent()
                .index();
              let rows = $(buildingTable).find(
                "#ally_content tr:nth-child(n+2)"
              );
              if (playerData[j].playerVillages.length == 0) {
                ...
                for (let p = 0; p < rows.length; p++) {
                  coordinate =
                    rows[p].children[0].innerText.match(/\d+\|\d+/)[0];
                  replaceVillages.push({
                    coordinate: coordinate,
                    currentPop: 0,
                    totalPop: 0,
                    attacksToVillage: "---",
              ...
              for (let r = 0; r < rows.length; r++) {
                if (rows[r].children[cellIndex].innerText.trim() != 0) {
                  playerData[j].playerVillages[r]["watchtower"] = parseInt(
                    rows[r].children[cellIndex].innerText.trim()
                  );
                }
                playerData[j].playerVillages[r]["wall"] = parseInt(
                  rows[r].children[wallIndex].innerText.trim()
                );
              }
            } else {
              let rows = $(buildingTable).find(
              ...
              for (let r = 0; r < rows.length; r++) {
                if (playerData[j].playerVillages) {
                  ...
                  playerData[j].playerVillages[r]["wall"] = "---";
                }
              }
            }
          }
...
input[type=range]::-webkit-slider-thumb {
	...
	-webkit-appearance: none;
          ...
          function setLeftValue() {
            ...
            $(".track").css(
              "background-image",
              `linear-gradient(to right, #75FFFF, black ${
                (minimum / bigStack) * 100
              }%, black ${inputLeft.value - 10}%, red ${
                inputLeft.value
              }%, red ${inputRight.value}%, yellow ${
          ...
          function makeOutputContainer(data) {
            ...
            testVar = data;
            coordinate =
              data.xy.toString().substring(0, 3) +
              "|" +
              data.xy.toString().substring(3, 6);
            thisData = targetData.filter(function (coord) {
              return coord.coord == coordinate;
            });
            troopsHome = [];
            troopsEnRoute = [];
            if (thisData.length > 0) {
              if (Object.keys(thisData[0].unitsInVillage).length > 0)
                troopsHome = thisData[0].unitsInVillage;
              if (Object.keys(thisData[0].unitsEnRoute).length > 0)
                troopsEnRoute = thisData[0].unitsEnRoute;
        ...
        <div>`);
          ...
          function makeTroopTds(troops) {
            ...
            var parts = [];
            counts.forEach((cnt) =>
              parts.push(`<td>${cnt || cnt == 0 ? cnt : ""}</td>`)
            );
            return parts.join(" ");
          }
        },
...
function storeSettings() {
  ...
  playerData.forEach((el) => {
    ...
    el.checkedWT = $(`#checkMapWT${el.playerID.replace(/[\s()]/g, "")}`).is(
      ":checked"
    );
    el.checkedWTMini = $(
      `#checkWTMini${el.playerID.replace(/[\s()]/g, "")}`
    ).is(":checked");
    settingsData.push([
...
function updateSlider() {
  ...
  $(".track").css(
    ...
    `linear-gradient(to right, #75FFFF, black ${
      (minimum / bigStack) * 100
    }%, black ${inputLeft.value - 10}%, red ${inputLeft.value}%, red ${
      inputRight.value
    }%, yellow ${parseInt(inputRight.value) + 10}%, yellow 95% ,green)`
  );
}
...
function toggleUI() {
  if ($('#toggleIcon[src*="minus.png"]').length > 0)
    $("#toggleIcon")[0].src = "graphic/plus.png";
...
function createUIOverview() {
                    ...
                    <div id="playerSettings">
                        ...
                        <table class="vis overviewWithPadding" style="border: 1px solid #7d510f;min-width:600px;max-width: 900px;">
                            <thead><tr>
                                <th>Player name</th>
                                <th ${
                                  "watchtower" in game_data.village.buildings
                                    ? ""
                                    : 'style="display:none"'
                                } style="width:80px;text-align: center;">Map WT</th>
                                <th ${
                                  "watchtower" in game_data.village.buildings
                                    ? ""
                                    : 'style="display:none"'
                                } style="width:80px;text-align: center;">Minimap WT</th>
  ...
  playerData.forEach((el, i) => {
    ...
    if (
      settingsData.playerSettings == undefined ||
      settingsData.playerSettings[i] == undefined
    ) {
                        ...
                        <tr class="${rowClass}">
                            ...
                            <td ${
                              "watchtower" in game_data.village.buildings
                                ? ""
                                : 'style="display:none"'
                            }><center><input id="checkMapWT${el.playerID.replace(
      /[\s()]/g,
      ""
    )}" type="checkbox" ${checkedWT ? "checked" : ""}></input></center></td>
                            <td ${
                              "watchtower" in game_data.village.buildings
                                ? ""
                                : 'style="display:none"'
                            }><center><input id="checkWTMini${el.playerID.replace(
      /[\s()]/g,
      ""
    )}" type="checkbox" ${checkedWTMini ? "checked" : ""}></input></center></td>
                            <td><center><button class="btn" id="color${el.playerID.replace(
                              /[\s()]/g,
                              ""
                            )}" data-jscolor="{
                                valueElement:'#val${el.playerID.replace(
                                  /[\s()]/g,
                                  ""
                                )}',
                                alphaElement:'#alp${el.playerID.replace(
                                  /[\s()]/g,
                                  ""
                                )}'}"></button>
                            <input id="val${el.playerID.replace(
                              /[\s()]/g,
                              ""
                            )}" value="${el.color}" name="color${
      el.playerName
    }[value]" type="hidden">
                            <input id="alp${el.playerID.replace(
                              /[\s()]/g,
                              ""
                            )}" value="${el.opacity}" name="color${
      el.playerName
    }[alpha]" type="hidden">
                            </center></td>
  ...
  scriptUI += `
                    ...
                    <div id="stackSize">
                        ...
                        <table class="vis overviewWithPadding" style="border: 1px solid #7d510f;margin:20px;">
                            ...
                            <tbody>
                                <tr>
                                    <td align="center"><input type="text" data-pop="1" onchange="storeSettings();" name="spear" id="spear" size="2" value="${
                                      unitPopValues.spear
                                    }" value=""></td>
                                    <td align="center"><input type="text" data-pop="1" onchange="storeSettings();" name="sword" id="sword" size="2" value="${
                                      unitPopValues.sword
                                    }"></td>
                                    <td align="center"><input type="text" data-pop="1" onchange="storeSettings();" name="axe" id="axe" size="2" value="${
                                      unitPopValues.axe
                                    }"></td>
                                    <td align="center"><input type="text" data-pop="1" onchange="storeSettings();" name="archer" id="archer" size="2" value="${
                                      unitPopValues.archer
                                    }"></td>
                                    <td align="center"><input type="text" data-pop="2" onchange="storeSettings();" name="spy" id="spy" size="2" value="${
                                      unitPopValues.spy
                                    }"></td>
                                    <td align="center"><input type="text" data-pop="4" onchange="storeSettings();" name="light" id="light" size="2" value="${
                                      unitPopValues.light
                                    }"></td>
                                    <td align="center"><input type="text" data-pop="5" onchange="storeSettings();" name="marcher" id="marcher" size="2" value="${
                                      unitPopValues.marcher
                                    }"></td>
                                    <td align="center"><input type="text" data-pop="6" onchange="storeSettings();" name="heavy" id="heavy" size="2" value="${
                                      unitPopValues.heavy
                                    }"></td>
                                    <td align="center"><input type="text" data-pop="5" onchange="storeSettings();" name="ram" id="ram" size="2" value="${
                                      unitPopValues.ram
                                    }"></td>
                                    <td align="center"><input type="text" data-pop="8" onchange="storeSettings();" name="catapult" id="catapult" size="2" value="${
                                      unitPopValues.catapult
                                    }"></td>
                                    <td align="center"><input type="text" data-pop="1" onchange="storeSettings();" name="knight" id="knight" size="2" value="${
                                      unitPopValues.knight
                                    }"></td>
                                    <td align="center"><input type="text" data-pop="100" onchange="storeSettings();" name="snob" id="snob" size="2" value="${
                                      unitPopValues.snob
                                    }"></td>
                                    <td align="center"><input type="text" data-pop="1" onchange="storeSettings();" name="militia" id="militia" size="2" value="${
                                      unitPopValues.militia
                                    }"></td>
                                </tr>
                            </tbody>
                        </table>
                    ...
                    <div id="stackList">
                        <div style="width:600px; margin: 30px;">
                            ...
                            <p>
                                <textarea rows="${
                                  selectedVillages.length + 1 < 10
                                    ? selectedVillages.length + 1
                                    : 10
                                }" style="width:590px;max-height:155px;overflow-y: auto;" id="villageList" value=""></textarea>
                            </p>
                            <hr>
                            <p>
                                <textarea rows="${
                                  selectedVillages.length + 4 < 10
                                    ? selectedVillages.length + 4
                                    : 10
                                }" style="width:590px;max-height:155px;overflow-y: auto;" id="villageListBB" value=""></textarea>
                            </p>
                            <br />
                        </div>
                    </div>
    ...
    if (player.playerVillages) {
      player.playerVillages.forEach((village) => {
        ...
        game_data.units.forEach((element, j) => {
          currentPop =
            currentPop +
            village.unitsInVillage[element] * unitPopValues[game_data.units[j]];
          totalPop =
            totalPop +
            village.unitsInVillage[element] *
              unitPopValues[game_data.units[j]] +
            parseInt(village.unitsEnroute[element]) *
              unitPopValues[game_data.units[j]];
        });
        let wt;
              ...
              targetData.forEach((element) => {
                ...
                if (
                  ...
                  (originXY = mapOverlay.map.pixelByCoord(t[0], t[1])),
                    (originX =
                      originXY[0] - st_pixel[0] + mapOverlay.tileSize[0] / 2),
                    (originY =
                      originXY[1] - st_pixel[1] + mapOverlay.tileSize[1] / 2),
                  ...
                  //current
                  if (element.currentStack < minimum)
                    currentColor = "rgba(117, 255, 255, 0.5)";
                  else if (
                    element.currentStack < smallStack &&
                    element.currentStack > minimum
                  )
                    currentColor = "rgba(0, 0, 0, 0.5)";
                  else if (
                    element.currentStack > smallStack &&
                    element.currentStack < mediumStack
                  )
                    currentColor = "rgba(255, 0, 0, 0.5)";
                  else if (
                    element.currentStack > mediumStack &&
                    element.currentStack < bigStack
                  )
                    currentColor = "rgba(255, 255, 0, 0.5)";
                  else if (element.currentStack > bigStack)
                    currentColor = "rgba(0, 255, 0, 0.5)";
                  //total
                  if (element.totalStack < minimum)
                    totalColor = "rgba(117, 255, 255, 0.5)";
                  else if (
                    element.totalStack < smallStack &&
                    element.totalStack > minimum
                  )
                    totalColor = "rgba(0, 0, 0, 0.5)";
                  else if (
                    element.totalStack > smallStack &&
                    element.totalStack < mediumStack
                  )
                    totalColor = "rgba(255, 0, 0, 0.5)";
                  else if (
                    element.totalStack > mediumStack &&
                    element.totalStack < bigStack
                  )
                    totalColor = "rgba(255, 255, 0, 0.5)";
                  else if (element.totalStack > bigStack)
                    totalColor = "rgba(0, 255, 0, 0.5)";
                  ...
                  if (element.wall < 20 || element.wall == "---")
                    iconOnMap(images[1], canvas, originX + 7, originY - 12, 15);
...
function drawMapTowers(canvas, sector, x, y, wtLevel, color, opacity) {
  ...
  ctx.ellipse(
    x,
    y,
    wtr * TWMap.map.scale[0],
    wtr * TWMap.map.scale[1],
    0,
    0,
    2 * Math.PI
  );
...
TWMap.map._handleClick = function (e) {
  ...
  if (village && village.id) {
    ...
    if (!currentCoords.includes(coord)) {
      ...
      if (targetData) {
        ...
        if (filteredArray.length > 0) {
          filteredArray.forEach((element) => {
            if (element.length > 0) {
              stacklist += `Coordinate: ${element[0].coord} - Player:  ${
                element[0].playerName
              } - Tribe:  ${
                element[0].tribeName
              } - Current Stack: ${numberWithCommas(
                element[0].totalStack
              )} - Packets needed: ${Math.round(
                (targetStackSize - element[0].totalStack) / packetSize
              )}\n`;
              stacklistBB += `[*][coord]${element[0].coord}[/coord][|]${
                element[0].playerName
              }[|]${element[0].tribeName}[|]${numberWithCommas(
                element[0].totalStack
              )}[|]${Math.round(
                (targetStackSize - element[0].totalStack) / packetSize
              )}\n`;
      ...
      $("#villageList").attr(
        "rows",
        selectedVillages.length + 1 < 10 ? selectedVillages.length + 1 : 10
      );
      $("#villageListBB").attr(
        "rows",
        selectedVillages.length + 4 < 10 ? selectedVillages.length + 4 : 10
      );
