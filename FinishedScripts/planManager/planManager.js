...
var scriptConfig = {
  scriptData: {
    ...
    author: "SaveBank",
    authorUrl:
      "https://forum.tribalwars.net/index.php?members/savebank.131111/",
    helpLink:
      "https://forum.tribalwars.net/index.php?threads/attack-plan-manager.292267/",
  },
  translations: {
    en_DK: {
      ...
      "Please select more plans to combine":
        "Please select more plans to combine",
      Saved: "Saved",
    },
    de_DE: {
      ...
      "Exported and copied to clipboard":
        "Exportiert und in die Zwischenablage kopiert",
      "Combine Plan": "Kombinieren",
      "Please select more plans to combine":
        "Bitte wählen Sie mehr Pläne aus, um sie zu kombinieren",
      Saved: "Gespeichert",
    },
  },
  allowedMarkets: [],
...
$.getScript(
  ...
  async function () {
    ...
    const { worldUnitInfo, worldConfig, tribes, players, villages } =
      await fetchWorldConfigData();
    ...
    if (DEBUG)
      console.debug(
        `${scriptInfo}: Startup time: ${(endTime - startTime).toFixed(
          2
        )} milliseconds`
      );
    ...
    (async function () {
      try {
        ...
        if (DEBUG)
          console.debug(
            `${scriptInfo}: Time to initialize: ${(endTime - startTime).toFixed(
              2
            )} milliseconds`
          );
      } catch (error) {
      ...
      let content = `
                ...
                <div>
                    <fieldset class="ra-mb10 sb-grid sb-grid-20-60-40-20">
                        ...
                        <fieldset class="ra-mb10 sb-grid sb-grid-4">
                            ...
                            <div>
                                <button id="savePlan" class="btn ">${twSDK.tt(
                                  "Save Plan"
                                )}</button>
                            </div>
                            <div>
                                <button id="renamePlan" class="btn ">${twSDK.tt(
                                  "Rename Plan"
                                )}</button>
                            </div>
                            <div>
                                <button id="deletePlan" class="btn ">${twSDK.tt(
                                  "Delete Plan"
                                )}</button>
                            </div>
                            <div>
                                <button id="combinePlan" class="btn ">${twSDK.tt(
                                  "Combine Plan"
                                )}</button>
                            </div>
                        </fieldset>
                        <fieldset class="ra-mb10 sb-grid sb-grid-2">
                            ...
                            <div>
                                <label for="useTemplates">${twSDK.tt(
                                  "Use Troop Templates"
                                )}</label>
                                <input type="checkbox" id="useTemplates" name="useTemplates">
                            </div>
                            <div>
                                <button id="buttonLoadTemplates" class="btn ">${twSDK.tt(
                                  "Load Troop Templates"
                                )}</button>
                            </div>
                        </fieldset>
                        <div class="ra-tac">
                            <button id="resetInput" class="btn " >${twSDK.tt(
                              "Reset Input"
                            )}</button>
                        </div>
                    </fieldset>
                ...
                <div id="manageCommandsDiv" style="display: none;">
                    <fieldset class="ra-mb10 sb-grid sb-grid-17-17-67">
                        ...
                        <div>
                            <label for="sendByTime">${twSDK.tt(
                              "Send by Time (min)"
                            )}</label>
                            <input type="number" id="sendByTime">
                            <button id="buttonByTime" class="btn">${twSDK.tt(
                              "Send"
                            )}</button>
                        </div>
                        <div>
                            <label for="sendByNumber">${twSDK.tt(
                              "Send by Number"
                            )}</label>
                            <input type="number" id="sendByNumber">
                            <button id="buttonByNumber" class="btn">${twSDK.tt(
                              "Send"
                            )}</button>
                        </div>
                        <fieldset class="ra-mb10 sb-grid sb-grid-4">
                            ...
                            <div>
                                <button id="buttonDeleteSelected" class="btn ">${twSDK.tt(
                                  "Delete selected commands"
                                )}</button>
                            </div>
                            <div>
                                <button id="buttonDeleteSent" class="btn ">${twSDK.tt(
                                  "Delete sent commands"
                                )}</button>
                            </div>
                            <div>
                                <button id="buttonDeleteExpired" class="btn ">${twSDK.tt(
                                  "Delete expired commands"
                                )}</button>
                            </div>
                            <div>
                                <button id="buttonDeleteAll" class="btn ">${twSDK.tt(
                                  "Delete all commands"
                                )}</button>
                            </div>
                        </fieldset>
            ...
            `;
      twSDK.renderBoxWidget(content, "sbPlanManager", "sb-plan-manager", style);
    }
    ...
    function addEventHandlers() {
      ...
      $("#exportPlan").click(function () {
        ...
        let exportContent =
          sbPlans[parseInt(planId.substring(planId.lastIndexOf("-") + 1))];
      ...
      $("#savePlan").click(function () {
        ...
        if (DEBUG) {
          ...
          console.debug(
            `${scriptInfo} Details of the plan being saved: `,
      ...
      $("#deletePlan").click(function () {
        ...
        let actualPlanId = parseInt(planId.substring(lastDashIndex + 1));
        if (DEBUG)
          console.debug(`${scriptInfo} Deleting plan with ID: ${actualPlanId}`);

        deletePlan(actualPlanId);
        if (actualPlanId in sbPlans) {
          if (DEBUG) {
            console.debug(
              `${scriptInfo} Plan with ID: ${actualPlanId} exists in sbPlans`
            );
      ...
      $("#renamePlan").click(function () {
        ...
        if (planNames[planId]) {
          ...
          newPlanName = prompt(
            "Please enter the new name for the plan",
            savedPlanName
          );
          if (DEBUG)
            console.debug(
              `${scriptInfo} Renaming plan with ID: ${planId} from ${savedPlanName} to ${newPlanName}`
            );
        } else {
          newPlanName = prompt(
            "Please enter the new name for the plan",
          ...
          if (DEBUG)
            console.debug(
              `${scriptInfo} Naming new plan with ID: ${planId} as ${newPlanName}`
            );
        }
      ...
      $("#combinePlan").click(function () {
        ...
        let body =
          '<div class="ra-table-container"><table class="ra-mb10 ra-table" width="100%">';
        for (let i = 0; i < planIds.length; i++) {
          let optionValue = planIds[i];
          let optionText =
            planNames && planNames[planIds[i]]
              ? planNames[planIds[i]]
              : planIds[i];
        ...
        body += "</table></div>";
        body +=
          '<button id="combineButton" class="btn ra-mb10">Combine</button>';
        ...
        $("#combineButton").click(function () {
          const localStorageSettings = getLocalStorage();
          let selectedPlanIds = $(".plan-checkbox:checked")
            .map(function () {
              return this.id;
            })
            .get();
          ...
          if (DEBUG)
            console.debug(
              `${scriptInfo} Selected plan IDs: ${selectedPlanIds.join(", ")}`
            );

          let targetPlanId = selectedPlanIds.includes(
            localStorageSettings.planSelector
          )
            ? localStorageSettings.planSelector
            : selectedPlanIds[0];
          ...
          if (DEBUG)
            console.debug(`${scriptInfo} Target plan ID: ${targetPlanId}`);

          let actualTargetPlanId = parseInt(
            targetPlanId.substring(targetPlanId.lastIndexOf("-") + 1)
          );

          // Combine the plans into the target plan
          selectedPlanIds.forEach((planId) => {
            if (planId !== targetPlanId) {
              let actualPlanIdToCombine = parseInt(
                planId.substring(planId.lastIndexOf("-") + 1)
              );

              // Combine the commands into the target plan
              sbPlans[actualTargetPlanId] = sbPlans[actualTargetPlanId].concat(
                sbPlans[actualPlanIdToCombine]
              );
              ...
              if (DEBUG)
                console.debug(
                  `${scriptInfo} Combined plan ${planId} into ${targetPlanId}`
                );
            }
          });
      ...
      $("#buttonDeleteAll").click(function () {
        ...
        if (DEBUG)
          console.debug(
            `${scriptInfo} Deleting all commands for plan with ID: ${actualPlanId}`
          );
        ...
        for (let command of sbPlans[actualPlanId]) {
          ...
          if (DEBUG)
            console.debug(
              `${scriptInfo} Removed command with ID: ${command.trCommandId} from the table`
            );
        }
      ...
      $("#buttonDeleteExpired").click(function () {
        ...
        if (DEBUG)
          console.debug(
            `${scriptInfo} Deleting expired commands for plan with ID: ${actualPlanId}`
          );
        ...
        for (let rowId of rowsToRemove) {
          ...
          if (DEBUG)
            console.debug(
              `${scriptInfo} Removed command with ID: ${rowId} from the table`
            );
        }
      ...
      $("#buttonDeleteSent").click(function () {
        ...
        if (DEBUG)
          console.debug(
            `${scriptInfo} Deleting sent commands for plan with ID: ${actualPlanId}`
          );
      ...
      $("#buttonDeleteSelected").click(function () {
        ...
        for (let command of sbPlans[actualPlanId]) {
          if (
            command.checkboxId &&
            $(`#${command.checkboxId}`).is(":checked")
          ) {
            if (DEBUG)
              console.debug(
                `${scriptInfo} Checkbox with ID: ${command.checkboxId} is checked for deletion`
              );
        ...
        for (let row of rowsToDelete) {
          ...
          if (DEBUG)
            console.debug(
              `${scriptInfo} Removed row with ID: ${row.attr(
                "id"
              )} from the table`
            );
        }

        sbPlans[actualPlanId] = sbPlans[actualPlanId].filter(
          (command) => !commandsToDelete.includes(command)
        );
        if (DEBUG)
          console.debug(
            `${scriptInfo} Updated plan with ID: ${actualPlanId} after deleting selected commands`
          );
      ...
      $("#buttonByNumber").click(function () {
        ...
        if (DEBUG)
          console.debug(
            `${scriptInfo} Sorting commands for plan with ID: ${actualPlanId} by number: ${number}`
          );
        plan.sort((a, b) => {
          let distanceA = getDistanceFromIDs(
            parseInt(a.originVillageId),
            parseInt(a.targetVillageId)
          );
          let unitSpeedA = parseInt(worldUnitInfo.config[a.slowestUnit].speed);
          let sendTimestampA = parseInt(
            parseInt(a.arrivalTimestamp) -
              twSDK.getTravelTimeInSecond(distanceA, unitSpeedA) * 1000
          );
          let remainingTimestampA = parseInt(sendTimestampA - Date.now());

          let distanceB = getDistanceFromIDs(
            parseInt(b.originVillageId),
            parseInt(b.targetVillageId)
          );
          let unitSpeedB = parseInt(worldUnitInfo.config[b.slowestUnit].speed);
          let sendTimestampB = parseInt(
            parseInt(b.arrivalTimestamp) -
              twSDK.getTravelTimeInSecond(distanceB, unitSpeedB) * 1000
          );
        ...
        for (let command of plan) {
          commandsToSend.push(
            generateLink(
              parseInt(command.originVillageId),
              parseInt(command.targetVillageId),
              command.units,
              command.trCommandId,
              command.type
            )
        ...
        if (DEBUG)
          console.debug(
            `${scriptInfo} Generated ${commandsToSend.length} commands to send`
          );
        let delay = 200;
      ...
      $(document).ready(function () {
        ...
        if (DEBUG)
          console.debug(
            `${scriptInfo} Document is ready, attaching change handlers to IDs`
          );
        $.each(sbAllIdsAPM, function (index, id) {
          $("#" + id).on("change", handleInputChange);
          if (DEBUG)
            console.debug(`${scriptInfo} Attached change handler to ID: ${id}`);
        });
      });
    }
    ...
    function initializeInputFields() {
      ...
      getAllPlans()
        .then((plans) => {
          if (DEBUG)
            console.debug(`${scriptInfo} Retrieved ${plans.length} plans`);
          if (plans.length > 0) {
            for (let i = 0; i < plans.length; i++) {
              sbPlans[plans[i].key] = plans[i].plan;
              $("#sbPlansDiv").append(
                `<div id="plan-id-${plans[i].key}" class="ra-mb10">${renderPlan(
                  plans[i].plan,
                  plans[i].key
                )}</div>`
              );
              ...
              if (DEBUG)
                console.debug(
                  `${scriptInfo} Processed plan with key: ${plans[i].key}`
                );
            }
          }
          populatePlanSelector();
    ...
    function updateCommandCount(planId) {
      ...
      if (planId) {
        if (!sbPlans || !sbPlans[planId]) {
          if (DEBUG)
            console.warn(
              `${scriptInfo} Plan with ID: ${planId} does not exist`
            );
          return;
        }
        ...
        if (!sbPlans || !sbPlans[actualPlanId]) {
          if (DEBUG)
            console.warn(
              `${scriptInfo} Plan with ID: ${actualPlanId} does not exist`
            );
          return;
        }
        commandCount = sbPlans[actualPlanId].length;
        document.getElementById(`count${actualPlanId}`).textContent =
          commandCount;
      }
      if (DEBUG)
        console.debug(
          `${scriptInfo} Updated command count for plan with ID: ${actualPlanId}`
        );
      return commandCount;
    }
    ...
    function renderPlanRows(plan, id) {
      if (DEBUG)
        console.debug(
          `${scriptInfo} Rendering plan rows for plan with ID: ${id}`
        );
      plan.sort((a, b) => {
        let distanceA = getDistanceFromIDs(
          parseInt(a.originVillageId),
      ...
      for (let i = 0; i < plan.length; i++) {
        ...
        let distance = getDistanceFromIDs(
          parseInt(row.originVillageId),
          parseInt(row.targetVillageId)
        );
        let unitSpeed = parseInt(worldUnitInfo.config[row.slowestUnit].speed);
        let getTravelTimeInMS =
          twSDK.getTravelTimeInSecond(distance, unitSpeed) * 1000;
        let sendTimestamp = parseInt(
          parseInt(row.arrivalTimestamp) - getTravelTimeInMS
        );
        ...
        let arrivalTime = convertTimestampToDateString(
          parseInt(row.arrivalTimestamp)
        );
        ...
        if (DEBUG)
          console.debug(
            `${scriptInfo} Processed row with command ID: ${commandId} for plan with ID: ${id}`
          );

        tbodyContent += `
            <tr id="${trCommandId}">
                <td class="ra-tac"><input type="checkbox" id="${checkboxId}"></td>
                <td class="ra-tac"><a href="/game.php?village=${
                  game_data.village.id
                }&screen=info_village&id=${
          row.originVillageId
        }"><span class="quickedit-label">${
          villageMap.get(parseInt(row.originVillageId))[2]
        }|${villageMap.get(parseInt(row.originVillageId))[3]}</span></a></td>
                <td class="ra-tac">${
                  villageMap.get(parseInt(row.originVillageId))[4] !== 0
                    ? `<a href="/game.php?village=${
                        game_data.village.id
                      }&screen=info_player&id=${
                        villageMap.get(parseInt(row.originVillageId))[4]
                      }"><span class="quickedit-label">${
                        playersMap.get(
                          parseInt(
                            villageMap.get(parseInt(row.originVillageId))[4]
                    ...
                    : `<span class="quickedit-label">${twSDK.tt(
                        "Barbarian"
                      )}</span>`
                }</td>
                <td class="ra-tac"><a href="/game.php?village=${
          ...
          villageMap.get(parseInt(row.targetVillageId))[2]
        }|${villageMap.get(parseInt(row.targetVillageId))[3]}</span></td>
                <td class="ra-tac">${
                  villageMap.get(parseInt(row.targetVillageId))[4] !== 0
                    ? `<a href="/game.php?village=${
                        game_data.village.id
                      }&screen=info_player&id=${
                        villageMap.get(parseInt(row.targetVillageId))[4]
                      }"><span class="quickedit-label">${
                        playersMap.get(
                          parseInt(
                            villageMap.get(parseInt(row.targetVillageId))[4]
                ...
                <td class="ra-tac"><img src="https://dsde.innogamescdn.com/asset/9f9563bf/graphic/unit/unit_${
                  row.slowestUnit
                }.png"></td>
                <td class="ra-tac"><img src="${commandTypeImageLink}"></td>
                <td class="ra-tac">${sendTime}</td>
                <td class="ra-tac">${arrivalTime}</td>
                <td id="${timeStampId}" class="ra-tac"><span class="timer" data-endtime>${twSDK.secondsToHms(
          parseInt(remainingTimestamp / 1000)
        )}</span></td>
      ...
      if (DEBUG)
        console.debug(
          `${scriptInfo} Rendered plan rows for plan with ID: ${id}`
        );
      return tbodyContent;
    }
    ...
    function generateLink(
      ...
      if (DEBUG)
        console.debug(
          `${scriptInfo} Generating link for command from village ${villageId1} to village ${villageId2}`
        );
      let completeLink = getCurrentURL();
      completeLink +=
        twSDK.sitterId.length > 0
      ...
      let template = localStorageSettings.troopTemplates.find(
      ...
      if (DEBUG)
        console.debug(
          `${scriptInfo} Generated link for command from village ${villageId1} to village ${villageId2}`
        );
      return completeLink;
    }
      ...
      for (let i = 0; i < sbButtonIDsAPM.length; i++) {
        ...
        if (isSendButton) {
          ...
          sendButton.onclick = function () {
            let [planId, _, commandId] = buttonId
              .split("-")
            ...
            let originVillageId = parseInt(
              sbPlans[planId][key].originVillageId
            );
            let targetVillageId = parseInt(
              sbPlans[planId][key].targetVillageId
            );
            ...
            if (DEBUG)
              console.debug(
                `${scriptInfo} Sending command from village ${originVillageId} to village ${targetVillageId}`
              );
            let sendLink = generateLink(
              originVillageId,
              targetVillageId,
              units,
              trCommandId,
        ...
        if (isDeleteButton) {
          ...
          deleteButton.onclick = function () {
            ...
            if (plan) {
              let commandIndex = plan.findIndex(
            ...
            if (DEBUG)
              console.debug(
                `${scriptInfo} Deleted command with ID ${commandId} from plan ${planId}`
              );
            updateCommandCount();
          };
    ...
    function getDistanceFromIDs(originVillageId, targetVillageId) {
      let originVillage =
        villageMap.get(originVillageId)[2] +
        "|" +
        villageMap.get(originVillageId)[3];
      let targetVillage =
        villageMap.get(targetVillageId)[2] +
        "|" +
        villageMap.get(targetVillageId)[3];
    ...
    function generateImport() {
      const html = `
                ...
                <div class="ra-mb10">
                    <button id="importPlan" class="btn">${twSDK.tt(
                      "Import"
                    )}</button>
                </div>
            `;
    ...
    function generateExport() {
      const html = `
                ...
                <div class="ra-mb10">
                <button id="exportPlan" class="btn">${twSDK.tt(
                  "Export"
                )}</button>
            </div>
            `;
    ...
    function fillTemplateTable() {
      ...
      for (let commandType of commandTypes) {
        ...
        let commandTypeCell = $("<td></td>").html(
          `<img src="${commandTypeImageLink}">`
        );
        ...
        select.on("change", function () {
          ...
          fillTemplateTable();
          if (DEBUG)
            console.debug(`${scriptInfo} Selected option:`, selectedOption);
        });
        ...
        let selectedTemplate =
          troopTemplates.find(
            (template) =>
              template.name === localStorageSettings.templateSelections[id]
          ) || troopTemplates[0];
        if (selectedTemplate) {
          for (let unit in selectedTemplate.units) {
            ...
            let unitImage = $(
              `<img class="unitImage" src="/graphic/unit/unit_${unit}.png" alt="${unit}"> `
            );
            let unitPreview = $(`
                <div>
    ...
    function populatePlanSelector() {
      ...
      for (let i = 0; i < planIds.length; i++) {
        ...
        if (DEBUG)
          console.debug(
            `${scriptInfo} Added plan with ID ${planIds[i]} to the plan selector`
          );
      }
      for (let planid of planIds) {
        if (planid === localStorageSettings.planSelector) {
          ...
          if (DEBUG)
            console.debug(`${scriptInfo} Selected plan with ID ${planid}`);
        } else {
          $(`#${planid}`).hide();
        }
      }
    ...
    function importPlan(content) {
      ...
      addPlan(plan)
        .then((key) => {
          if (DEBUG)
            console.debug(
              `${scriptInfo} Plan added successfully with key: ${key}`
            );
          let localStorageSettings = getLocalStorage();
    ...
    function convertWBPlanToArray(plan) {
      ...
      for (let i = 0; i < planArray.length; i++) {
        ...
        if (DEBUG)
          console.debug(
            `${scriptInfo}: Plan object ${i} created: `,
            planObject
          );
      }

      if (DEBUG)
        console.debug(`${scriptInfo}: Plan objects created: `, planObjects);
      return planObjects;
    }
    ...
    function count() {
      ...
      try {
        $.getJSON(
          `${apiUrl}/${namespace}/${apiKey}_id${encodedPlayerId}/up`,
          (response) => {
            if (response.count === 1) {
            ...
            if (DEBUG)
              console.debug(
                `Player ${playerId} script runs: ${response.count}`
              );
          }
        ).fail(() => {
    ...
    function openDatabase() {
      ...
      openRequest.onupgradeneeded = function (event) {
        ...
        if (!db.objectStoreNames.contains("Plans")) {
          let objectStore = db.createObjectStore("Plans", {
            autoIncrement: true,
          });
          if (DEBUG)
            console.debug(`${scriptInfo} Created new object store 'Plans'`);
        }
      };
    }
      ...
      return new Promise((resolve, reject) => {
        ...
        openRequest.onsuccess = function (event) {
          ...
          addRequest.onsuccess = function (event) {
            if (DEBUG)
              console.debug(
                `${scriptInfo} Plan added successfully with ID: ${event.target.result}`
              );
    ...
    function modifyPlan(planId, plan) {
      ...
      openRequest.onsuccess = function (event) {
        ...
        putRequest.onsuccess = function (event) {
          if (DEBUG)
            console.debug(
              `${scriptInfo} Plan modified successfully with ID: ${planId}`
            );
        };

        putRequest.onerror = function (event) {
          console.error(
            `${scriptInfo} Error modifying plan with ID: ${planId}`,
            event
          );
        };
      };
        ...
        openRequest.onsuccess = function (event) {
          ...
          cursorRequest.onsuccess = function (event) {
            ...
            if (cursor) {
              ...
              if (DEBUG)
                console.debug(
                  `${scriptInfo} Plans retrieved successfully:`,
                  plans
                );
              resolve(plans);
            }
    ...
    function deletePlan(planId) {
      ...
      openRequest.onsuccess = function (event) {
        ...
        deleteRequest.onsuccess = function (event) {
          if (DEBUG)
            console.debug(
              `${scriptInfo} Plan deleted successfully with ID: ${planId}`
            );
        };

        deleteRequest.onerror = function (event) {
          console.error(
            `${scriptInfo} Error deleting plan with ID: ${planId}`,
            event
          );
        };
      };
    ...
    async function getTroopTemplates() {
      ...
      if (game_data.player.sitter > 0) {
        baseUrl =
          getCurrentURL() +
          `?village=${game_data.village.id}&screen=place&mode=templates&t=${game_data.player.id}`;
      } else {
        baseUrl =
          getCurrentURL() +
          `?village=${game_data.village.id}&screen=place&mode=templates`;
      }
      ...
      let templateText = text
        .split("TroopTemplates.current = ")[1]
        .split(";\n\tTroopTemplates.deleteLink =")[0];
      let troopTemplatesRaw = JSON.parse(templateText) ?? {};

      if (DEBUG)
        console.debug(`${scriptInfo}: Troop templates: `, troopTemplatesRaw);
      ...
      for (let templateKey in troopTemplatesRaw) {
        ...
        if (DEBUG)
          console.debug(
            `${scriptInfo}: Troop template ${template.name}: `,
            templateUnits
          );
      }
      if (DEBUG)
        console.debug(`${scriptInfo}: Troop templates: `, troopTemplates);
      let localStorageSettings = getLocalStorage();
    ...
    function resetInput() {
      ...
      if (DEBUG)
        console.debug(
          `${scriptInfo} Input reset successfully, new settings:`,
      ...
      initializeInputFields();
      if (DEBUG)
        console.debug(`${scriptInfo} UI re-rendered after input reset`);
    }
    ...
    function handleInputChange() {
      ...
      if (DEBUG)
        console.debug(`${scriptInfo}: ${inputId} changed to ${inputValue}`);
    ...
    function getLocalStorage() {
      const localStorageSettings = JSON.parse(
        localStorage.getItem("sbPlanManager")
      );
      ...
      if (localStorageSettings) {
        missingSettings = expectedSettings.filter(
          (setting) => !(setting in localStorageSettings)
        );
        if (DEBUG && missingSettings.length > 0)
          console.debug(
            `${scriptInfo}: Missing settings in localStorage: `,
            missingSettings
          );
      }
      ...
      if (localStorageSettings && missingSettings.length === 0) {
        if (DEBUG)
          console.debug(
            `${scriptInfo}: Local storage settings retrieved successfully:`,
            localStorageSettings
          );
        ...
        if (DEBUG)
          console.debug(
            `${scriptInfo}: Default settings saved to local storage:`,
            defaultSettings
          );
    ...
    function saveLocalStorage(settingsObject) {
      ...
      if (DEBUG)
        console.debug(
          `${scriptInfo}: Settings saved to local storage:`,
          settingsObject
        );
    }
