...
if (
  ...
  (window.location.href.indexOf("screen=ranking") > -1 &&
    window.location.href.indexOf("&mode") == -1)
) {
  //any player
  tribeTable = ".vis:eq(2)";
  rowStart = 1;
...
$("#contentContainer")
  .eq(0)
  .prepend(
    "<span>BB</span><br><textarea id='BBCODE' rows='" +
      (rows + 4) +
...
$("#contentContainer")
  .eq(0)
  .prepend(
    "<span>CSV</span><br><textarea id='CSVCODE' rows='" +
      (rows + 4) +
      "' cols='100'></textarea><br>"
  );
function getData() {
  ...
  for (var i = 0; i < Object.keys(statsEnabled).length; i++) {
    ...
    console.log(
      "Set to: " + $(`:checkbox#${Object.keys(statsEnabled)[i]}`).is(":checked")
    );
    statsEnabled[Object.keys(statsEnabled)[i]] = $(
      `:checkbox#${Object.keys(statsEnabled)[i]}`
    ).is(":checked");
  }
  localStorage.setItem("tribeStatsEnabled", JSON.stringify(statsEnabled));
  ...
  for (var i = 0; i < names.length; i++) {
    if (statsEnabled.ODA)
      linksODA.push(
        "/game.php?screen=ranking&mode=in_a_day&type=kill_att&name=" + names[i]
      );
    if (statsEnabled.ODD)
      linksODD.push(
        "/game.php?screen=ranking&mode=in_a_day&type=kill_def&name=" + names[i]
      );
    if (statsEnabled.ODS)
      linksODS.push(
        "/game.php?screen=ranking&mode=in_a_day&type=kill_sup&name=" + names[i]
      );
    if (statsEnabled.Loot)
      linksLoot.push(
        "/game.php?screen=ranking&mode=in_a_day&type=loot_res&name=" + names[i]
      );
    if (statsEnabled.Gathering)
      linksGathering.push(
        "/game.php?screen=ranking&mode=in_a_day&type=scavenge&name=" + names[i]
      );
    if (statsEnabled.ODATotal)
      linksODATotal.push(
        "/game.php?screen=ranking&mode=kill_player&type=att&name=" + names[i]
      );
    if (statsEnabled.ODDTotal)
      linksODDTotal.push(
        "/game.php?screen=ranking&mode=kill_player&type=def&name=" + names[i]
      );
  ...
  $.getAll(
    ...
    () => {
      $("#progress").css(
        "width",
        `${(linksODA.length / linksODA.length) * 100}%`
      );
      for (var o = rowStart; o < ODAperPlayer.length + rowStart; o++) {
        $(tribeTable + " tr")
          ...
          .append(
            "<td title=" +
              ODAperPlayer[o - rowStart][1] +
              ">" +
              ODAperPlayer[o - rowStart][0] +
              "</td>"
          );
      }
      //ODD
        ...
        () => {
          for (var o = rowStart; o < ODDperPlayer.length + rowStart; o++) {
            $(tribeTable + " tr")
              ...
              .append(
                "<td title=" +
                  ODDperPlayer[o - rowStart][1] +
                  ">" +
                  ODDperPlayer[o - rowStart][0] +
                  "</td>"
              );
          }
          //ODS
            ...
            () => {
              for (var o = rowStart; o < ODSperPlayer.length + rowStart; o++) {
                $(tribeTable + " tr")
                  ...
                  .append(
                    "<td title=" +
                      ODSperPlayer[o - rowStart][1] +
                      ">" +
                      ODSperPlayer[o - rowStart][0] +
                      "</td>"
                  );
              }
              ...
              $.getAll(
                ...
                () => {
                  for (
                    var o = rowStart;
                    o < lootperPlayer.length + rowStart;
                    o++
                  ) {
                    $(tribeTable + " tr")
                      .eq(o)
                  ...
                  $.getAll(
                    ...
                    () => {
                      for (
                        var o = rowStart;
                        o < gatheredperPlayer.length + rowStart;
                        o++
                      ) {
                        $(tribeTable + " tr")
                          .eq(o)
                        ...
                        if (
                          statsEnabled["Loot"] == true &&
                          statsEnabled["Gathering"] == true
                        ) {
                          $(tribeTable + " tr")
                            .eq(o)
                            .append(
                          ...
                          if (statsEnabled["Combined"] == true)
                            $(tribeTable + " tr")
                              .eq(o)
                              .append(
                                "<td>Need both Loot and Gathering enabled to see this column</td>"
                              );
                        }
                      }
                      // ODA total
                      $.getAll(
                        ...
                        () => {
                          for (
                            var o = rowStart;
                            o < ODATotalperPlayer.length + rowStart;
                            o++
                          ) {
                            $(tribeTable + " tr")
                              .eq(o)
                          ...
                          $.getAll(
                            ...
                            () => {
                              for (
                                var o = rowStart;
                                o < ODDTotalperPlayer.length + rowStart;
                                o++
                              ) {
                                $(tribeTable + " tr")
                                  .eq(o)
                              ...
                              $.getAll(
                                ...
                                (i, data) => {
                                  if (
                                    $(data).find(".lit-item")[3] != undefined
                                  ) {
                                    temp = $(data).find(".lit-item");
                                    x = temp[3].innerText;
                                    console.log(x);
                                ...
                                () => {
                                  for (
                                    var o = rowStart;
                                    o < ODSTotalperPlayer.length + rowStart;
                                    o++
                                  ) {
                                    $(tribeTable + " tr")
                                      .eq(o)
...
for (var i = 0; i < Object.keys(statsEnabled).length; i++) {
  html += `
                <th align="center" style="width:50px;padding: 5px;">${
                  Object.keys(statsEnabled)[i]
                }</th>
            `;
}
html += "</tr><tr>";
for (var i = 0; i < Object.keys(statsEnabled).length; i++) {
  html += `
                <td align="center" style="padding: 5px;"><input type="checkbox" ID="${
                  Object.keys(statsEnabled)[i]
                }" name="${Object.keys(statsEnabled)[i]}"></td>
            `;
}
...
for (var i = 0; i < Object.keys(statsEnabled).length; i++) {
  $(`#${Object.keys(statsEnabled)[i]}`).prop(
    "checked",
    statsEnabled[Object.keys(statsEnabled)[i]]
  );
}
