  ...
  async function () {
    ...
    function onInitFindUntaggedIncomings(processedIncomings) {
      setTimeout(() => {
        ...
        if (untaggedIncomingsHaveBeenFound) {
          UI.ErrorMessage(twSDK.tt("Untagged incomings have been found!"));
        }
      }, 1000);
    }
    ...
    function handleOpSpotter(processedIncomings) {
      jQuery("#raOpSpotterBtn").on("click", function (e) {
        ...
        const sortedArrivalTimesCount = Object.entries(arrivalTimesCount).sort(
          (a, b) => b[1] - a[1]
        );
    ...
    function handleWatchtowerTimer() {
      jQuery("#raWatchtowerTimerBtn").on("click", function (e) {
        ...
        if (parseInt(worldConfig.config.game.watchtower)) {
          if (jQuery("#incomings_table tr:eq(0) th").length === 7) {
            ...
            UI.ErrorMessage(
              twSDK.tt("Watchtower Timer script is already initialized!")
            );
          }
        } else {
          UI.ErrorMessage(
      ...
      jQuery("#raFakeFinderBtn").on("click", function (e) {
        ...
        jQuery("#raFakeFinderDoBtn").on("click", function (e) {
          ...
          if (originCoordinates && originCoordinates.length) {
            ...
            const { incomingsGrouppedByAttackingVillages, incomingsObject } =
              processedIncomings;
                  ...
                  if (originCoord === coord) {
                    incomings.forEach((incomingId) => {
                      const { containsNoble, attackType } =
                        mappedIncomings[incomingId]["metadata"];
                      if (!containsNoble && attackType !== "attack_large.png") {
                        jQuery(
            ...
            if (affectedRows > 0) {
              ...
              UI.SuccessMessage(twSDK.tt("No fakes have been found!"));
            }
          } else {
            UI.ErrorMessage(twSDK.tt("At least one coordinate is required!"));
          }
        });
      });
    }
    ...
    function handleToggleCombinations(processedIncomings) {
      jQuery("#raToggleCombinationsBtn").on("click", function (e) {
        ...
        const destinationVillagesHtml = Object.values(uniqueDestinationVillages)
            ...
            .then((response) => {
              ...
              for (const [key, value] of Object.entries(units)) {
                unitsCounts.push({
                  unit: key,
                  count:
                    parseInt(value.count.home) + parseInt(value.count.foreign),
                });
              }
              ...
              jQuery(`tr.village-id-${villageId} td.ra-building-wall`).html(
                wallLevel
              );
              jQuery(`tr.village-id-${villageId} td.ra-building-farm`).html(
                farmLevel
              );
    ...
    function handleFilterIncomings(processedIncomings) {
      jQuery("#raFilterIncomingsBtn").on("click", function (e) {
        ...
        const selectDestinationVillageOptionsHtml = uniqueDestinationVillages
          .map((player) => {
    ...
    function handleDoFilterIncomings(processedIncomings) {
      jQuery("#raFilterIncomingsDoBtn").on("click", function (e) {
        ...
        let incomingsOnChosenDestinationVillage =
          incomingsGrouppedByDestinationVillages[chosenDestinationVillage] ?? [
            ...incomingIds,
          ];
        ...
        let filteredIncomingsByLandingTime = Object.values(incomingsObject)
          .filter((incoming) => {
            ...
            const landingTimeObject = new Date(getDate(landingTime, "full"));
        ...
        if (filteredIncomings.length) {
          ...
          UI.ErrorMessage(
            twSDK.tt("No incoming found that could fulfill all the criteria!")
          );
        }
      });
    }
      ...
      jQuery("#raMarkDuplicatesBtn").on("click", function (e) {
        ...
        Object.values(duplicateIncomings).forEach((incomings) => {
          incomings.forEach((incomingId, index) => {
            let nameOfIncomingCommand = incomingsMappedObject[incomingId].label;
            nameOfIncomingCommand = nameOfIncomingCommand
              .split("AS:")[0]
              .trim();
            nameOfIncomingCommand = `${nameOfIncomingCommand} AS:${index + 1}/${
              incomings.length
            }`;
        ...
        commands.forEach((command) => {
          ...
          Object.values(incomingsObject).forEach((incoming) => {
            ...
            if (coord === origin) {
              ...
              const returnTimeInt = getLandingTime(returnTime).getTime() / 1000;
              const gapBetweenTimes = landingTimeInt - returnTimeInt; // seconds

              if (gapBetweenTimes <= BACKTIME_GAP && gapBetweenTimes >= 0) {
                foundBacktimes.push({
                ...
                <table width="100%" class="ra-table ra-attack-types-table">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ${
                                  attackTypesFrequency["attack_large.png"] ?? 0
                                }/${totalIncomingAttacks}
                            </td>
                            <td>
                                ${twSDK.calculatePercentages(
                        ...
                        <tr>
                            ...
                            <td>
                                ${
                                  attackTypesFrequency["attack_medium.png"] ?? 0
                                }/${totalIncomingAttacks}
                            </td>
                            <td>
                                ${twSDK.calculatePercentages(
                        ...
                        <tr>
                            ...
                            <td>
                                ${
                                  attackTypesFrequency["attack_small.png"] ?? 0
                                }/${totalIncomingAttacks}
                            </td>
                            <td>
                                ${twSDK.calculatePercentages(
    ...
    function collectIncomingsList() {
      ...
      jQuery("#incomings_table tbody tr.nowrap").each((_, incoming) => {
        ...
        const incomingData = {
          id: incomingId,
          label: jQuery(incoming).find("span.quickedit-label").text().trim(),
          attacker: {
            id: parseInt(
              jQuery(incoming).find("td:eq(3) a").attr("href").split("id=")[1]
            ),
          ...
          origin: {
            id: parseInt(
              jQuery(incoming).find("td:eq(2) a").attr("href").split("id=")[1]
            ),
            coord: jQuery(incoming)
              .find("td:eq(2)")
              .text()
    ...
    function processIncomings(incomingsObject) {
      ...
      let attackingPlayerNames = attackingPlayers.map((player) => player.name);
      let attackingVillagesIds = attackingVillages.map((village) => village.id);
    ...
    function groupIncomingsByVillage(incomings, villages, type) {
      ...
      villages.forEach((village) => {
        ...
        incomings.forEach((incoming) => {
          ...
          const { id: destinationId, coord: destinationCoord } = destination;
          ...
          if (type === "origin") {
            ...
            if (village.id === destinationId) {
              incomingsByVillage.push(incomingId);
              grouppedIncomings.set(destinationCoord, incomingsByVillage);
            }
          }
        });
      });
    ...
    function getArrivalTimes(arrivals) {
      ...
      arrivals.forEach((arrival) => {
        ...
        arrivalTimes.push(`"${arrivalDate}/${arrivalMonth} ${arrivalHour}H"`);
      });
        ...
        if (tableRows.length) {
          tableRows.each(function () {
            const origin = jQuery(this)
              ...
              .text()
              .match(twSDK.coordsRegex)[0];
            const arrivalTime = jQuery(this).find("td:eq(2)").text().trim();
            ...
            jQuery(this)
              .find("td.unit-item")
              .each(function () {
                commandUnits.push(parseInt(jQuery(this).text().trim()));
              });
