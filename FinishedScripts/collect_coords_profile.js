if (game_data.player.premium == false) {
  alert("Para utilizar esse script Ã© necessÃ¡rio uma Conta Premium.");
  end();
}
...
if (document.URL.indexOf("screen=info_player") == -1) {
  ...
  for (var idx = 0; idx < tds.length; idx++) {
    ...
    if (/^\d+\|\d+$/.test(xy)) {
      ...
      var xys = xy.split("|");
      K[
        Math.floor(parseInt(xys[0]) / 100) +
          Math.floor(parseInt(xys[1]) / 100) * 10
      ].push(xy);
    }
  }
  if (bb == true) {
  ...
  var S =
    ...
    "</head>" +
    "<body>" +
    "<b>Coletor de Coordenadas</b><hr>Todas as Aldeias do Jogador:<br>" +
    prefix +
    C +
    postfix;
