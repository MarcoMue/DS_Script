      ...
      switch (screen) {
        case "overview":
          if (win.showVillageOverviewWideget && DBInfo.checkConfig()) {
            const coords = $(".box #menu_row2")[0]
              .innerText.split("(")[1]
              .split(")")[0]
              .split("|");
            DBInfo.showDatabaseDetails(
              coords[0],
              coords[1],
              DBInfo.openWidget,
              null
        ...
        case "overview_villages":
          if (win.showButtonOnAttackScreen && DBInfo.checkConfig()) {
            if (
              (mode === "incomings" || $("#show_units").length) &&
              $(
                "#paged_view_content > table.vis.modemenu > tbody > tr > td.selected"
              )[1].innerText === "Angriffe"
            ) {
        ...
        case "info_village":
          if (win.showVillageDetails && DBInfo.checkConfig()) {
            const coords =
              $(".vis")[0].children[0].children[2].children[1].innerText.split(
                "|"
              );
        ...
        case "settings":
          if ($("table.modemenu td.selected a").length > 0) {
            if (
              $("table.modemenu td.selected a")
                .attr("href")
                .includes("mode=settings")
            ) {
              const settingsElm = $("#content_value tr>td:last-child h3")[0];
              settingsElm.insertAdjacentHTML(
                "afterend",
                `
                            <div>
  ...
  function initDBInfo() {
    DBInfo = {
                    ...
                    <div class='popup_box show'>
                        ...
                        <div class='popup_box_content'><div class='center'>
                            ...
                            <table class='vis' width='100%' style='vertical-align:middle'>
                                ...
                                <tr>
                                    <td>API-Schl\u00FCssel:</td><td><input id='dbapikey' type='text' value="${
      ...
      showDatabaseDetails(x, y, callback, additionals) {
        ...
        var url =
          localStorage.getItem("dbMode") === "SF"
            ? win.serverConfig.sfAPI
            : win.serverConfig.userAPI;
      ...
      unitNameToSaveImg(unitName) {
        if (unitName == "Ramme")
          return `<img src='${DBInfo.dsCDNUnit("ram")}'>`;
        if (unitName == "Kata")
          return `<img src='${DBInfo.dsCDNUnit("catapult")}'>`;
        if (
          unitName == "spy" ||
          unitName == "Sp\u00E4her" ||
          unitName == "Sp\u00E4h"
        )
          return `<img src='${DBInfo.dsCDNUnit("spy")}'>`;
        if (unitName == "Axt") return `<img src='${DBInfo.dsCDNUnit("axe")}'>`;
        if (unitName == "Speer")
          return `<img src='${DBInfo.dsCDNUnit("spear")}'>`;
        if (unitName == "Schwert")
          return `<img src='${DBInfo.dsCDNUnit("sword")}'>`;
        if (unitName == "AG" || unitName == "snob")
          return `<img src='${DBInfo.dsCDNUnit("snob")}'>`;
        if (unitName == "Lkav")
          return `<img src='${DBInfo.dsCDNUnit("light")}'>`;
        if (unitName == "Skav")
          return `<img src='${DBInfo.dsCDNUnit("heavy")}'>`;
      ...
      generateUnitTableRow(unitsRaw, sanitizedType, time, rowCount, lossType) {
        ...
        DBInfo.supported_units.forEach((unit) => {
          ...
          unitData +=
            "<td>" + DBInfo.escapeHTML(unitsRaw[accessPart]) + "</td>";
        });
      ...
      generateIncTable(data) {
        ...
        data.sos.forEach((element, idx) => {
          ...
          var actualLink =
            ...
            element.attacker_koords_id;
          var unitSanitized = DBInfo.unitNameToSaveImg(element.Unit);
          rows += `
                    <tr class='nowrap row_${idx % 2 == 0 ? "a" : "b"}'>
                        <th>${idx + 1}</th>
                        <td>${unitSanitized}</td>
                        <td><a href='${actualLink}'>${DBInfo.escapeHTML(
            element.attacker_koords
          )}</a></td>
                        <td>${DBInfo.timeConverter(element.fighttime)}</td>
                        <td><span class='timer'>${DBInfo.getFormatedTimeLeftToUnixTimestamp(
                          element.fighttime
                        )}</span></td>
                        <td>${incType}</td>
      ...
      open(data, x, y) {
        ...
        var villagetype =
          data.type == 0 ? "Deff" : data.type == 1 ? "Off" : "Unbekannt";
        var linkToAllVillageDetails = win.serverConfig.villageDetail.replace(
          "$$village$$",
          DBInfo.escapeHTML(data.village_id)
        );
        output +=
          "<div>Dorftyp: <b>" +
          villagetype +
          "</b><br /><a href='" +
          linkToAllVillageDetails +
          "' target='_blank'>Datenbank Dorfakte</a></div><br />";

        let rowCount = 0;
        let rows = "";
        if (localStorage.getItem("dbMode") === "SF" && data.unit_time) {
          rows += DBInfo.generateUnitTableRow(
            data,
            "Im Dorf",
            data.unit_time,
            rowCount
        ...
        if (data.attack_report.fighttime) {
          const linkToAttackReport = win.serverConfig.reportPage.replace(
            "$$reportID$$",
            DBInfo.escapeHTML(data.attack_report.report_id)
          );
          const typeInfoAtt =
            "<a href='" +
            linkToAttackReport +
            "' target='_blank'>Letzter Angriff</a>";
          rows += DBInfo.generateUnitTableRow(
            data.attack_report,
            typeInfoAtt,
            data.attack_report.fighttime,
            rowCount,
            false
          );
          rowCount++;
          ...
          const typeInfoLoss =
            "<a href='" +
            linkToAttackReport +
            "' target='_blank'>Verlust Angriff</a>";
          rows += DBInfo.generateUnitTableRow(
            data.attack_report,
            typeInfoLoss,
            data.attack_report.fighttime,
        ...
        if (data.defend_report.fighttime) {
          const linkToDefendReport = win.serverConfig.reportPage.replace(
            "$$reportID$$",
            DBInfo.escapeHTML(data.defend_report.report_id)
          );
          const typeInfoDef =
            "<a href='" +
            linkToDefendReport +
            "' target='_blank'>Letzte Verteidigung</a>";
          rows += DBInfo.generateUnitTableRow(
            data.defend_report,
            typeInfoDef,
            data.defend_report.fighttime,
            rowCount,
            false
          );
          rowCount++;
        ...
        output +=
          "<div><p>Eingehende Angriffe: <b>" +
          DBInfo.escapeHTML(data.attacks) +
          "</b></p></div>";
        ...
        if (data.report_id) {
          const linkToReport = win.serverConfig.reportPage.replace(
            "$$reportID$$",
            DBInfo.escapeHTML(data.report_id)
          );
          output +=
            "<br /><div>Letzter Report: <a href='" +
            linkToReport +
            "' target='_blank'>" +
            DBInfo.timeConverter(data.report_fighttime) +
            "</a></div>";
        }

        var linkToAllReports = win.serverConfig.allReportsPage
          .replace("$$x$$", DBInfo.escapeHTML(x))
          .replace("$$y$$", DBInfo.escapeHTML(y));
        output +=
          "<div><a href='" +
          linkToAllReports +
          "' target='_blank'>Alle Reports</a></div>";
      ...
      openWidget(data) {
        $("#show_summary")[0].insertAdjacentHTML(
          "afterend",
          `
                <div>
      ...
      openAttackDetail() {
        ...
        // add row to header
        $(
          "#incomings_table tr:first-child th:last-child"
        )[0].insertAdjacentHTML("afterend", "<th>DB-Info</th>");
        ...
        for (let index = 1; index < incTable.length - 1; index++) {
          ...
          element.insertAdjacentHTML(
            ...
            `
                    <td>
                        <img src='https://dsde.innogamescdn.com/graphic/overview/note.png'
                            onclick='DBInfo.showDatabaseDetails(${coords[0]}, ${coords[1]}, DBInfo.showAttackDetail, "${arrival}")'>
                    </td>
                    `
          );
        }
      ...
      showAttackDetail(data, x, y, arrival) {
        var possibleAttacks = data.sos.filter((element) =>
          DBInfo.timeConverter(element.fighttime).includes(arrival)
        );
        ...
        possibleAttacks.forEach((element) => {
          ...
          message =
            "Angriff von: " +
            DBInfo.escapeHTML(element.attacker_koords) +
            " Typ: " +
            DBInfo.escapeHTML(incType) +
            " Grund: " +
            DBInfo.escapeHTML(element.reason) +
            "\n";
        });
        if (message) {
