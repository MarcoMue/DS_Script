javascript: if (window.location.href.indexOf("&screen=main") < 0) {
  //relocate
...
function checkBuildings() {
  ...
  for (var i = 0; i < $("#buildings tr .build_options .inactive").length; i++) {
    ...
    wood = $("#buildings tr .build_options .inactive")
      .eq(i)
      .parents()
      .eq(1)
      .find($("[data-cost]"))
      ...
      .text()
      .trim();
    stone = $("#buildings tr .build_options .inactive")
      .eq(i)
      .parents()
      .eq(1)
      .find($("[data-cost]"))
      ...
      .text()
      .trim();
    iron = $("#buildings tr .build_options .inactive")
      .eq(i)
      .parents()
      .eq(1)
      .find($("[data-cost]"))
    ...
    if (
      $("#buildings tr .build_options .inactive").eq(i).text() !=
      "The Warehouse is too small"
    ) {
      $("#buildings tr .build_options .inactive")
        .eq(i)
        .parent()
        .parent()
        .append(
          `<td id="request${i}"><input type="button" class="btn evt-confirm-btn btn-confirm-yes" id="calculate${i}" onclick="requestRes(${i})" value="Request"></td>`
        );
    } else {
      $("#buildings tr .build_options .inactive")
        .eq(i)
        ...
        .parent()
        .append(
          `<td id="request${i}"><input type="button" class="btn btn-disabled" id="calculate" value="WH CAP"></td>`
        );
    }
  }
}
...
function requestRes(buildingNr) {
  resource[sourceID] = {};
  resource[sourceID]["wood"] = Math.max(
    0,
    resourcesNeeded[buildingNr].wood - nonReservedWood
  );
  resource[sourceID]["stone"] = Math.max(
    0,
    resourcesNeeded[buildingNr].stone - nonReservedStone
  );
  resource[sourceID]["iron"] = Math.max(
    0,
    resourcesNeeded[buildingNr].iron - nonReservedIron
  );
  //check if this would cause an overflow
  if (
    WHWoodCap + resource[sourceID]["wood"] > WHCap ||
    WHStoneCap + resource[sourceID]["stone"] > WHCap ||
    WHIronCap + resource[sourceID]["iron"] > WHCap
  ) {
    ...
    TribalWars.post(
      ...
      { "select-village": sourceID, target_id: 0, resource: resource },
      function (e) {
        UI.SuccessMessage(`
            <p>
            <span>Resources requested: </span>
            <span class="icon header wood"></span><span>${
              resource[sourceID]["wood"]
            }</span>
            <span class="icon header stone"></span><span>${
              resource[sourceID]["stone"]
            }</span>
            <span class="icon header iron"></span><span>${
              resource[sourceID]["iron"]
            }</span>
            </p>
            <p>
            <span>Used/reserved WH space: </span>
            <span class="icon header wood"></span><span>${
              resource[sourceID]["wood"] + WHWoodCap
            }</span>
            <span class="icon header stone"></span><span>${
              resource[sourceID]["stone"] + WHStoneCap
            }</span>
            <span class="icon header iron"></span><span>${
              resource[sourceID]["iron"] + WHIronCap
            }</span>
            </p>
            `);
        nonReservedWood =
          nonReservedWood +
          resource[sourceID]["wood"] -
          resourcesNeeded[buildingNr].wood;
        nonReservedStone =
          nonReservedStone +
          resource[sourceID]["stone"] -
          resourcesNeeded[buildingNr].stone;
        nonReservedIron =
          nonReservedIron +
          resource[sourceID]["iron"] -
          resourcesNeeded[buildingNr].iron;
...
function showSourceSelect() {
  ...
  sources = [];
  $.get(
    "/game.php?&screen=overview_villages&mode=prod&group=0&page=-1&",
    function (resourcePage) {
      ...
      $.each(rowsResPage, function (index) {
        ...
        tempWood = $(rowsResPage[index].children[3])
          .find(".wood")
          .text()
          .replace(".", "");
        tempStone = $(rowsResPage[index].children[3])
          .find(".stone")
          .text()
          .replace(".", "");
        tempIron = $(rowsResPage[index].children[3])
          .find(".iron")
          .text()
          .replace(".", "");
        tempVillageID = $(rowsResPage)
          .eq(index)
          .find("span[data-id]")
          .attr("data-id");
        tempVillageName = $(rowsResPage)
          .eq(index)
    ...
    $.each(sources, function (ind) {
      htmlSelection += `
            <tr class="trclass" style="cursor: pointer" onclick="storeSourceID(${
              sources[ind].id
            },'${sources[ind].name}',${sources[ind].wood},${
        sources[ind].stone
      },${sources[ind].iron},${sources[ind].merchants.match(/(\d+)\//)[1]})">
