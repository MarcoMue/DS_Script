...
function calculateEverything() {
  ...
  $.getAll(
    ...
    (i, data) => {
      ...
      for (
        var pages = 0;
        pages < $(data).find(".paged-nav-item").length / 2;
        pages++
      ) {
      ...
      $.getAll(
        ...
        () => {
          console.log(
            "Rows for player " + player[i].name + " total: " + rows.length
          );
          ...
          $.each(rows, function (rowNr) {
            ...
            $.each(game_data.units, function (index) {
              ...
              if (
                ...
                villageData[thisID][unitName] = rows
                  .eq(rowNr)
                  .children()
                  .not(":first")
                  .eq(index)
                  .text()
                  .trim();
                villageData["total"][unitName] += parseInt(
                  rows
                    .eq(rowNr)
                    .children()
                    .not(":first")
...
function displayEverything() {
  ...
  $.each(playerData, function (playerName) {
    ...
    for (
      ...
      villageCounter < Object.keys(playerData[playerName]).length;
      villageCounter++
    ) {
      if (Object.keys(playerData[playerName])[villageCounter] != "total") {
          ...
          switch (
            Object.keys(
              playerData[playerName][
                Object.keys(playerData[playerName])[villageCounter]
              ]
            )[lol]
          ) {
            case "spear":
              thisVillageDefPop += parseInt(
                playerData[playerName][
                  Object.keys(playerData[playerName])[villageCounter]
                ][game_data.units[lol]]
              );
        ...
        if (thisVillageOffPop < fullPop && thisVillageOffPop >= almostPop)
          typeTotals[playerName]["almostNuke"] += 1;
        if (thisVillageOffPop < almostPop && thisVillageOffPop >= semiPop)
          typeTotals[playerName]["semiNuke"] += 1;
        if (thisVillageOffPop < semiPop && thisVillageOffPop >= quarterPop)
          typeTotals[playerName]["quarterNuke"] += 1;

        // Def
        if (thisVillageDefPop >= fullPop) typeTotals[playerName]["fullDV"] += 1;
        if (thisVillageDefPop < fullPop && thisVillageDefPop >= almostPop)
          typeTotals[playerName]["almostDV"] += 1;
        if (thisVillageDefPop < almostPop && thisVillageDefPop >= semiPop)
          typeTotals[playerName]["semiDV"] += 1;
        if (thisVillageDefPop < semiPop && thisVillageDefPop >= quarterPop)
    ...
    $.each(playerData[playerName]["total"], function (troopName) {
      ...
      html += `<td><table><tr><td class="item-padded"><img src="/graphic/unit/unit_${troopName}.png" title="${troopName}" alt="" class=""></td>
                <td class="item-padded">${numberWithCommas(
                  playerData[playerName]["total"][troopName]
                )}</td></tr></table></td>`;
    });
