javascript: if (document.URL.match("mode=incomings&subtype=attacks")) {
  $("#incomings_table")
    .find("tr")
    .eq(0)
    .find("th")
  ...
  var url =
      ...
      "overview_villages&mode=buildings&group=0&page=-1",
    url2 = "https://" + location.host + "/interface.php?func=get_unit_info",
    url2 = "https://de228.die-staemme.de/interface.php?func=get_unit_info",
    towerCoords = [],
    towerLevels = [],
    unitSpeed = [],
    intersectionPoints = [],
  ...
  function first() {
    $.ajax({
      ...
      success: function (data) {
        console.log(data);
        $.each(
          ["sword", "axe", "spy", "light", "heavy", "ram", "snob"],
          function (key, val) {
            // extract unit speeds
            unitSpeed.push(
              Number(
                $(data)
                  .find("config > " + val + " > speed")
                  .text()
              ) * 60
            );
          }
          ...
          success: function (datas) {
            $(datas)
              ...
              .each(function (key, val) {
                if (
                  Number($(val).find(".upgrade_building.b_watchtower").text()) >
                  0
                ) {
                  ...
                  var level = Number(
                    $(val).find(".upgrade_building.b_watchtower").text()
                  );
                  switch (level) {
                    case 1:
            ...
            if (towerCoords.length == 0) {
              UI.ErrorMessage(
                "There are no watchtowers in any of your villages!",
                5000
              );
            }
          },
  ...
  var doStuff = function () {
    ...
    var destination = $("#incomings_table")
      .find("tr")
      .eq(timesRun)
      .find("td")
      .eq(1)
      ...
      if (d >= 0) {
        ...
        if (d == 0) {
          ...
          intersectionPoints.push(
            Number(intersections[0]) + "|" + Number(m * intersections[0] + n)
          );
        }
        ...
        intersectionPoints.push(
          Number(intersections[1]) + "|" + Number(m * intersections[1] + n)
        );
      }
      // nothing in common (?)
    }
    ...
    for (var i = 0; i < intersectionPoints.length; i++) {
      ...
      var originDistance = Math.sqrt(
        Math.pow(intersections[0] - source[0], 2) +
          Math.pow(intersections[1] - source[1], 2)
      );
    ...
    var M = nearest.split("|");
    var remaining =
      Math.sqrt(Math.pow(M[0] - source[0], 2) + Math.pow(M[1] - source[1], 2)) -
      currentDistance;
    if (commandName.includes("sword")) {
  ...
  self.location = game_data.link_base_pure.replace(
    /screen\=\w*/i,
    "screen=overview_villages&mode=incomings&subtype=attacks"
  );
}
void 0;
